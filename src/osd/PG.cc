...
void PG::scrub(ThreadPool::TPHandle &handle)
{
  ...
  if (!scrubber.active) {
    ...
    for (unsigned i=1; i<acting.size(); i++) {
      ...
      if (!con)
	continue;
      if (!(con->get_features() & CEPH_FEATURE_CHUNKY_SCRUB)) {
  ...
  : my_base(ctx)
{
  ...
  if (con) {
    if ((con->get_features() & CEPH_FEATURE_BACKFILL_RESERVATION)) {
...
PG::RecoveryState::WaitRemoteRecoveryReserved::react(const RemoteRecoveryReserved &evt) {
  ...
  if (acting_osd_it != context< Active >().sorted_acting_set.end()) {
    ...
    if (con) {
      if ((con->get_features() & CEPH_FEATURE_RECOVERY_RESERVATION)) {
	pg->osd->send_message_osd_cluster(
